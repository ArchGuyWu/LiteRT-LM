...
// Custom renderer to render code blocks as litert-code-block elements.
const customRenderer = new marked.Renderer();
customRenderer.code = (arg1: unknown, arg2?: string) => {
  const code = typeof arg1 === 'string' ? arg1 : (arg1 as {text: string}).text;
  let language = typeof arg1 === 'string' ? arg2 : (arg1 as {lang?: string}).lang;
...
const markedOptions = { renderer: customRenderer };
...
export class LitertChatBubble extends LitElement {
  ...
  private renderMarkdown(text: string): string {
    ...
    if (!text) return '';
    try {
      return marked.parse(text, markedOptions) as string;
    } catch (e) {
